...
public abstract class XmlTagBinder<T> implements ButterKnife.ViewBinder<T>  {
    protected ButterKnife.ViewBinder<T> mBinderWrapper;

    public XmlTagBinder(ButterKnife.ViewBinder<T> binderWrapper) {
        mBinderWrapper = binderWrapper;
    }

    @Override
    public void bind(final ButterKnife.Finder finder, final T target, Object source) {
        mBinderWrapper.bind(finder, target, source);
    }

    @Override
    public void unbind(T target) {
        mBinderWrapper.unbind(target);
    }
}
